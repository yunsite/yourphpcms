...
function template_file($module='',$path='',$ext='html'){
	...
	$path= $path ? $path : TMPL_PATH.$sysConfig['DEFAULT_THEME'].'/Home/';
...
function make_urlrule($url,$module,$action,$MOREREQUEST=''){
		...
		$rule=str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}','/'),array('([A-Za-z0-9]+)','([A-Z]{1}[a-z]+)','(\d+)','([A-Za-z0-9]+)','(\d+)','(\d+)','(\d+)','(\d+)','(\d+)','(\d+)','\/'),$url);

		if(APP_LANG){
			$Lang=F('Lang');
			$sysConfig =F('sys.config');
			foreach((array)$Lang as $key =>$r){
				if($sysConfig['DEFAULT_LANG']!=$key){
					$langrule = $key.'\/';
					$langrequest = 'l='.$key;
				}
				$urlrule='array(\'/^'.$langrule.$rule.'$/\',\''.$module.'/'.$action.'\',\''.$REQUEST.'\',\''.$MOREREQUEST.$langrequest.'\')';
...
function routes_cache($URL_URLRULE=''){
			...
			$urlstr .=  'array(\'/^Tags$/\',\'Home/Tags/index\',\'\'),'."\n";
			$urlstr .=  'array(\'/^Tags\/(\d+).html$/\',\'Home/Tags/index\',\'p\'),'."\n";
			if(APP_LANG){
				$Lang=F('Lang');
				foreach((array)$Lang as $key =>$r){$langarr[]=$key;}
				$urlstr .=  'array(\'/^('.implode('|',$langarr).')$/\',\'Home/index/index\',\'l\',\'\'),'."\n";				
				$urlstr .=  'array(\'/^('.implode('|',$langarr).')\/Tags$/\',\'Home/Tags/index\',\'l\'),'."\n";
				$urlstr .=  'array(\'/^('.implode('|',$langarr).')\/Tags\/(\d+).html$/\',\'Home/Tags/index\',\'l,p\'),'."\n";
			}
...
function TAGURL($data,$p=''){
	...
	if(C('URL_M')==0){
			...
			$langurl = $lang ? $lang.'/' : '' ;
			$url=$index.$lang.'Tags'.$module.$tag.'/';
...
function getlang($have=''){
	if($have){
		if(strcasecmp(GROUP_NAME,'Admin')== 0)
			$lang =  $_SESSION['YP_lang'];
		else
			$lang =  $_REQUEST['l'] ? $_REQUEST['l'] : C('URL_LANG');
	}else{
		if(strcasecmp(GROUP_NAME,'Admin')== 0)
			$lang =  C('URL_LANG')!=$_SESSION['YP_lang'] ? $_SESSION['YP_lang'] : '';
		else
...
function geturl($cat,$data='',$Urlrule=''){
		...
		if($cat['ishtml']){
			...
			if($URL_MODEL==0){
				$langurl = $lang ?  '&l='.$_SESSION['YP_lang'] : '';
				if($id){
...
function return_url($code){		
		$config = APP_LANG ?  F('Config_'.LANG_NOW) :  F('Config');
