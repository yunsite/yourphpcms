...
class AdminbaseAction extends Action
{
	...
	function _initialize()
	{
		...
		if(APP_LANG){
			...
			if($_GET['l']){
				...
				Session::set('YP_lang',$_GET['l']);	
				Session::set('YP_langid',$this->Lang[$_GET['l']]['id']);
				}else{
				$this->error ( L ( 'NO_LANG' ) );
				}
			}
			if(!$_SESSION['YP_lang'] || !$_SESSION['YP_langid']){
				Session::set('YP_lang',$this->sysConfig['DEFAULT_LANG']);
				Session::set('YP_langid',$this->Lang[$this->sysConfig['DEFAULT_LANG']]['id']);	
			}
			define('LANG_NOW',$_SESSION['YP_lang']);
 
			$this->assign('l',$_SESSION['YP_lang']);
			$this->assign('langid',$_SESSION['YP_langid']);
			$this->categorys = F('Category_'.$_SESSION['YP_lang']);
			$this->Config = F('Config_'.$_SESSION['YP_lang']);
	...
	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15) {
		...
		if(APP_LANG)if($this->moduleid)$map['lang']=array('eq',$_SESSION['YP_langid']);
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			$lang =  C('URL_LANG')!=$_SESSION['YP_lang'] ? $lang = $_SESSION['YP_lang'].'/' : '';
			L(include LANG_PATH.$_SESSION['YP_lang'].'/common.php');
			$T = F('config_'.$_SESSION['YP_lang'],'', './Yourphp/Tpl/'.$this->sysConfig['DEFAULT_THEME'].'/Home/'); 
		}else{
	...
	public function create_index()
    {
		...
		$r=$this->buildHtml('index','./'.$lang,'Yourphp/Tpl/'.$this->sysConfig['DEFAULT_THEME'].'/Home/Index_index');
		if($r) return true;
    }
