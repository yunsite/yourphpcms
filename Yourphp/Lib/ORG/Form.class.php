...
class Form extends Think {
    ...
    public function __construct($data=array()) {
		 ...
		 if(APP_LANG){ 
			if(strcasecmp(GROUP_NAME,'Admin')== 0){
				$this->lang = $_SESSION['YP_lang'];
			}else{
				$this->lang = $_REQUEST['l'] ? $_REQUEST['l'] : C('DEFAULT_LANG');
			}
		 }
    }
	...
	public function title($info,$value){
		...
		$thumbstr ='<div class="thumb_box"  id="thumb_box"><div id="thumb_aid_box"></div>
				<a href="javascript:swfupload(\'thumb_uploadfile\',\'thumb\',\''.L('uploadfiles').'\','.$this->isadmin.',1,1,1,\'jpeg,jpg,png,gif\','.$info['setup']['upload_maxsize'].','.$info['moduleid'].',\''.$yourphp_auth.'\',yesdo,nodo)"><img src="'.$thumb_ico.'" id="thumb_pic" ></a><br> <input type="button" value="'.L('clean-thumb').'" onclick="javascript:clean_thumb(\'thumb\');" class="button" />
