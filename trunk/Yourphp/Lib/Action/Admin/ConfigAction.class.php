...
class ConfigAction extends AdminbaseAction {
	...
	public function index() {
		...
		foreach($config as $key=>$r) {
			...
			if($r['groupid']==2){
				if(APP_LANG){
					if($r['lang']==$_SESSION['YP_langid']) $this->site_config[$r['varname']]=$r;
				}else{
	...
	public function member() {
		...
		if(APP_LANG)$where = ' and lang='.$_SESSION['YP_langid']; 
		$config = $this->dao->where("groupid=3".$where)->findAll();
	...
	public function mail() {
 	...
 	public function dosite() {
		...
		if(APP_LANG && $_POST['site_name'])$where = ' and lang='.$_SESSION['YP_langid'];
		...
		if($f){ 
			if($this->Lang[$_POST['DEFAULT_LANG']]['id']){
				Session::set('YP_lang',$_POST['DEFAULT_LANG']);
				Session::set('YP_langid',$this->Lang[$_POST['DEFAULT_LANG']]['id']);
			}
